...
(function () {
  ...
  TV.widget('#w-reduce', {
    ...
    hint: 'ดูว่ามุม θ (สีอำพัน) ไปอยู่ตรงไหนของสามเหลี่ยมรูปใหม่: กลุ่ม 180° กับ 360° θ ยังอยู่ที่จุดกำเนิดเหมือนเดิม ชื่อฟังก์ชันจึงคงเดิม · แต่กลุ่ม 90° กับ 270° θ ย้ายไปอยู่ที่จุด P′ ด้านประกอบจึงสลับหน้าที่กัน ชื่อฟังก์ชันเลยเปลี่ยนเป็นโคฟังก์ชัน (รูปนี้ตั้ง θ เป็นมุมแหลมเพื่อให้เห็นภาพชัด ส่วนการพิสูจน์ที่ครอบคลุมทุกค่าของ θ ใช้สูตรผลบวกมุมจากหัวข้อ 4.3)',
    onPointer: function (p, s, wx, wy) {
    ...
    draw: function (p, s, api) {
      ...
      /* ---- θ ตัวเดิมไปโผล่ที่มุมไหนของสามเหลี่ยมรูปใหม่ ---- */
      var vx = d.sw ? q[0] : 0, vy = d.sw ? q[1] : 0;   /* จุดยอดที่มุม θ ไปอยู่ */
      var e0, e1;
      if (d.sw) {
        /* ที่จุด P′ : วัดจากด้านตั้งฉาก (ชี้ไปหาเท้าบนแกน x) ไปหาด้านตรงข้ามมุมฉาก (ชี้ไปหาจุดกำเนิด) */
        e0 = q[1] >= 0 ? -PI / 2 : PI / 2;
        e1 = Math.atan2(-q[1], -q[0]);
      } else {
        /* ที่จุดกำเนิด : วัดจากแกน x ฝั่งที่เท้าอยู่ ไปหา P′ */
        e0 = q[0] >= 0 ? 0 : PI;
        e1 = Math.atan2(q[1], q[0]);
      }
      while (e1 - e0 > PI) e1 -= TAU;
      while (e0 - e1 > PI) e1 += TAU;
      var er = d.sw ? 0.17 : 0.2;
      p.arc(vx, vy, er, e0, e1, { fill: 'rgba(180,83,9,.16)', color: C.accent, w: 1.6 });
      var em = (e0 + e1) / 2;
      p.text(vx + er * cos(em), vy + er * sin(em), 'θ', {
        dx: 19 * cos(em), dy: -19 * sin(em), color: C.accent, size: 12.5, bg: 'rgba(255,255,255,.85)'
      });
